...
(defn get-poll
  "Takes :id and/or :title, and returns the first matched poll if any matched the passed keywords,
  otherwise nil"
  [& {:keys [id title] :as kwmap}]
  (let [polls (map deref @polls) ; Deref it
        non-nil-keys (sort (keys kwmap))
        _ (assert (not (empty? non-nil-keys)))
        get-relv #(apply only-keys %1 non-nil-keys)
        first-match (some #(when (= kwmap (get-relv %)) %) polls)]
    first-match))
